...
{
   curLetter = 0;
   tmpWord = arrWords[random(arrWords.length)];
}
function newLetter()
{
   curLetter++;
   attachMovie("char","char" + curLetter,curLetter);
   this["char" + curLetter].char.char.txt.text = tmpWord.substr(curLetter - 1,1);
   this["char" + curLetter]._x = random(100) + 400;
   this["char" + curLetter]._y = -100;
   this["char" + curLetter].char.char.txt.autoSize = "left";
}
function removeGood(obj)
{
   port.door.tween("_y",(- 88 / tmpWord.length) * curLetter,1);
   S_gateopen.start();
   strWord.text = strWord.text + tmpWord.substr(curLetter - 1,1);
   if(curLetter < tmpWord.length)
   {
      newLetter();
   }
   else
   {
      removeMovieClip(starRain);
      play();
   }
   removeMovieClip(obj);
}
function removeBad(obj)
{
   strWord.text = "";
   removeMovieClip(obj);
   ranWord();
   newLetter();
   port.door.tween("_y",0,0.2);
   S_gateclose.start();
}
...
delete mp;
this.createEmptyMovieClip("mc_wolves",100000);
this.createEmptyMovieClip("mc_bgS",2);
S_wolves = new Sound("mc_wolves");
S_wolves.attachSound("wolves");
S_wolves.start(0,999);
S_bg = new Sound("mc_bgS");
S_bg.attachSound("S_bgSound");
S_bg.start(0,999);
S_bg.setVolume(60);
S_wolves.setVolume(50);
S_gateopen = new Sound(this);
S_gateopen.attachSound("gateOpen");
S_gateclose = new Sound(this);
S_gateclose.attachSound("gateClose");
var speed = 2;
var i = 1;
var root;
var numLetters = 10;
...
var timer = 0;
var timeSpace = 20;
arrWords = new Array("SPÖKHUS","ALBATROSS","KRÅKSLOTT","BOLIBOMPA","PIRATSKEPP","FLADDERMUS","SPINDEL","FULLMÅNE","PORTVAKT","UGGLA");
i = 0;
this.createEmptyMovieClip("starRain",100);
starRain.onEnterFrame = function()
{
   this.attachMovie("miniStar_trail","miniStar_trail" + i,i);
   this["miniStar_trail" + i]._x = _X + random(150) + 400;
   this["miniStar_trail" + i]._y = -30;
   scale = random(40) + 70;
   this["miniStar_trail" + i]._xscale = scale;
   this["miniStar_trail" + i]._yscale = scale;
   animTime = 2 / (scale / 50);
   this["miniStar_trail" + i].tween(["_y","_alpha"],[_Y + 100,0],1 + animTime,"easeinsine",undefined,function()
   {
